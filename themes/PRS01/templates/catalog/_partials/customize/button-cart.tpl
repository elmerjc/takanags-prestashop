	...
	{if !$configuration.is_catalog}
		<form action="{$urls.pages.cart}" method="post" class="add-to-cart-or-refresh">
			...
			{if $product.quantity > 0 && $product.quantity >= $product.minimal_quantity}
				<a href="javascript:void(0);" class="button ajax_add_to_cart_button add-to-cart btn-default" data-button-action="add-to-cart" title="{l s='Add to cart'}">
					<span>{l s='Add to cart' d='Shop.Theme.Actions'}</span>
				</a>
			{else}
				<a href="javascript:void(0);" class="button ajax_add_to_cart_button add-to-cart-disable btn-default" title="{l s='Out of stock'}">
