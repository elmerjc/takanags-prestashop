 ...
 <section class="brands container">
	...
	{if $brandname}
		<h1 class="h3 product-title" itemprop="name">
			<a class="product-name" style="color: #FFF" itemprop="url"  href="{$link->getmanufacturerLink($brand['id_manufacturer'], $brand['link_rewrite'])}" title="{$brand.name}">{$brand.name}</a>
		</h1>
	{/if}
	</li>
	{/foreach}
